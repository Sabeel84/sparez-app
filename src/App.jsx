...
export default function App(){
  ...
  const openListing=(l)=>{setActiveListing(l);setScreen("listing");};
  ...
  const myChats=Object.keys(chats).filter(k=>{const[b,s]=k.split("_");return b===currentUser?.id||s===currentUser?.id;});
  ...
  return(
    <div style={{background:"#f5f5f5",minHeight:"100vh",maxWidth:430,margin:"0 auto",position:"relative",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      ...
      {screen==="listing"    &&activeListing&&<ListingScreen listing={activeListing} currentUser={currentUser} onBack={()=>setScreen("home")} openChat={openChat} markSold={markSold} deleteListing={deleteListing} openRating={()=>setRatingModal({sellerId:activeListing.sellerId,sellerName:activeListing.sellerName})} users={users} viewCur={viewCur} setViewCur={setViewCur} chats={chats}/>}
      {screen==="addlisting" &&<AddListingScreen currentUser={currentUser} setListings={setListings} notify={notify} setScreen={setScreen}/>}
      {screen==="chat"       &&activeChatKey&&<ChatScreen chatKey={activeChatKey} currentUser={currentUser} chats={chats} setChats={setChats} onBack={()=>setScreen("home")} openRating={()=>setRatingModal({sellerId:activeChatKey.listing.sellerId,sellerName:activeChatKey.listing.sellerName})} users={users}/>}
